...
export async function generateMetadata({
  ...
  const dict = (await getDictionary(locale, 'privacy')) as PrivacyDict;
    ...
    >
      {sections.map((section, index) => (
        <section key={index} className={index < sections.length - 1 ? 'mb-8' : ''}>
          ...
          {section.items && (
            <ul className={`text-sm sm:text-base text-gray-700 space-y-2 list-disc list-inside ${section.content ? 'mt-4' : ''}`}>
