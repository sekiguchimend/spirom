      ...
      <article className="min-h-screen bg-[#FFFFF5]">
        <div className="max-w-4xl mx-auto pt-24 sm:pt-16 pb-16 px-4">
          ...
          <header className="mb-8">
            ...
            {post.category && (
              <Link
                href={`${routes.BLOG.INDEX}?category=${post.category.slug.current}`}
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-gray-900">
                    <Link href={`${routes.BLOG.INDEX}?author=${post.author.slug?.current}`} className="hover:underline">
                      {post.author.name}
                    </Link>
                  </p>
                  {post.author.bio && (
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-12">
              {post.tags.map((tag) => (
                <Link
                  key={tag}
                  href={`${routes.BLOG.INDEX}?tag=${tag}`}
            ...
            <div className="max-w-7xl mx-auto px-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {relatedPosts.map((relatedPost) => (
                  <ContentCard
                    key={relatedPost._id}
                    href={routes.BLOG.DETAIL(relatedPost.slug.current)}
