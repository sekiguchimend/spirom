...
export function CategoryPill({ href, children, isActive = false }: CategoryPillProps) {
  return (
    <Link
      href={href}
      className={`inline-flex items-center justify-center px-6 py-2.5 text-sm font-black uppercase tracking-wider border-3 border-black rounded-full shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] transition-all duration-200 ${
        isActive
          ? 'bg-black text-white'
