...
pub async fn auth_middleware(
    ...
    let claims = validate_supabase_token(&token, &state.config.jwt.secret)?;

    let user: AuthenticatedUser = claims.try_into()
...
pub async fn optional_auth_middleware(
    ...
    if let Ok(token) = extract_token(&request) {
        if let Ok(claims) = validate_supabase_token(&token, &state.config.jwt.secret) {
...
fn validate_supabase_token(token: &str, secret: &str) -> Result<Claims, AppError> {
    let decoding_key = DecodingKey::from_secret(secret.as_bytes());

    // Supabase AuthはHS256を使用
    let mut validation = Validation::new(Algorithm::HS256);
    // Supabase Authのaudience
    validation.set_audience(&["authenticated"]);
    // issuerの検証はスキップ（複数環境対応のため）
